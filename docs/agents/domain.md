...
├── CONTEXT.md
├── docs/adr/
│   ├── 0001-supabase-rls-as-the-authorization-boundary.md
│   └── 0002-static-html-no-build-step.md
└── supabase/
```

Neither `CONTEXT.md` nor `docs/adr/` exists yet — the filenames above are illustrative.

If this repo ever grows into a multi-package monorepo, the alternative layout is a root
`CONTEXT-MAP.md` pointing at one `CONTEXT.md` per context, with context-scoped
`src/<context>/docs/adr/`. Re-run `/setup-matt-pocock-skills` to switch.
